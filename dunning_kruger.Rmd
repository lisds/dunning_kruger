...
```{python}
percentile_actual = np.linspace(0, 100, 10000)
bottom_quartile = 10
top_quartile = -5
# Adjust perceived ability based on quartiles
new_perceived = np.zeros_like(percentile_actual)
new_perceived[:2500] += bottom_quartile
new_perceived[7500:] += top_quartile
```

```{python}
# Calculate the difference between perceived and actual percentiles
perceived_minus_actual = new_perceived - percentile_actual

# Create a DataFrame
df = pd.DataFrame({
    'actual_percentile': percentile_actual,
    'perceived_percentile': new_perceived,
    'difference': perceived_minus_actual
})
```

```{python}
# Categorize actual percentiles into quantiles
df['actual_quantile'] = pd.qcut(df['actual_percentile'], 4, labels=['low', 'med-low', 'med-high', 'high'])
```
...
# Calculate the mean difference in perceived ability for each quantile
mean_difference_by_quantile = df.groupby('actual_quantile', observed=True)['difference'].mean()
print(mean_difference_by_quantile)

# Plot the results
plt.scatter(df['actual_percentile'], df['perceived_percentile'], label='Perceived vs Actual')
plt.xlabel('Actual Percentile')
plt.ylabel('Perceived Percentile')
plt.legend()
plt.title('Perceived vs Actual Percentiles')
plt.show()
```
