...
```{python}
#Percentiles from a test:
actualPercentiles = np.linspace(0, 100, 10_000)
```

```{python}
#Perceived percentile performance in a test:
perceivedPercentiles = actualPercentiles + 10 #BTA effect
perceivedPercentiles = pd.Series(perceivedPercentilesBTA).apply(lambda x: 80 if x > 80 else 20 if x < 20 else x) #Crushing
```

```{python}
perceivedMinusActual = perceivedPercentiles - actualPercentiles
actualMinusPerceived = actualPercentiles - perceivedPercentiles
```

```{python}
plt.plot(perceivedPercentiles,label='Perceived')
plt.plot(actualPercentiles,label='Actual')
plt.fill_between(perceivedPercentiles.index,
                 perceivedPercentiles,
                 actualPercentiles,
                 where=(perceivedPercentiles > actualPercentiles), color='red', alpha=0.3)
plt.fill_between(perceivedPercentiles.index,
                 perceivedPercentiles,
                 actualPercentiles,
                 where=(perceivedPercentiles < actualPercentiles), color='green', alpha=0.3)
```
