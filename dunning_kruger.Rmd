...
## First update - add some randomness to the percentiles


Maybe adding some randomness will attenuate the DK effect?

(Pause - is that really likely to be true?)

Notice that we are still using the crushed BTA estimates.

```{python}
# add some randomness to the the perceived percentiles
df['perceived_percentile_w_randomness'] = df['perceived_percentile'] + rng.normal(0, 20, len(df))
